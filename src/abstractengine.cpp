...
AbstractEngine* AbstractEngineFactory::getAbstractEngineInstance(
  const string& filename, const string& type, int ntasks_per_worker) {
 ...
 GreasyLog::getInstance()->record(GreasyLog::devel, "AbstractEngineInstance::getAbstractEngineInstance", "Entering with type: '" + type+"'");
  ...
  return new BasicEngine(filename, ntasks_per_worker);
  }
...
AbstractEngine::AbstractEngine (string filename, int ntasks_per_worker) {
...
void AbstractEngine::finalize() {
  ...
  std::string command = "if [ ! -z ${CRAY_CUDA_MPS+x} ]; then /opt/cray/pdsh/default/bin/pdsh -w $SLURM_NODELIST /etc/init.d/cray-nvidia-cuda-mps stop; fi";
